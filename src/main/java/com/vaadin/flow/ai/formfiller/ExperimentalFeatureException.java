...
public class ExperimentalFeatureException extends RuntimeException {
    public ExperimentalFeatureException() {
        super("The Form Filler add-on is currently an experimental feature and needs to be explicitly enabled. " +
              "It may be removed, altered, or limited to commercial subscribers in future releases. " +
              "The add-on can be enabled using the Vaadin Dev Tools window, " +
              "in the experimental features tab, or by adding a `src/main/resources/vaadin-featureflags.properties` " +
              "file with the following content: `com.vaadin.experimental.formFillerAddon=true`");
    }
}
